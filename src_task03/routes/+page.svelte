...
<script lang="ts">
    ...
    // let randomNums2:number= Math.floor(Math.random()*10)
    for(let i = 0 ; i < 6 ; i++){
        randomNums[i] = (Math.floor(Math.random()*1000)*10);
    }
    console.log(`randomNumList: ${randomNums}`);

    let dataList:number[] = randomNums;
    let heightList:number[] = getHeight(dataList);
    let ScaleList: number[] = getScale(dataList);
    let nameList: string[] = ['A','B','C','D','E','F'];
...
<div class="displayScreen">
    ...
    <button class="redrawBtn"  on:click={()=>{window.location.reload();}}>REDRAW</button>
</div>
